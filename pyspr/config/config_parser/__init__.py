...
def parse_config(git_cmd: GitInterface) -> Config:
    ...
    try:
        with open('.spr.yaml', 'r') as f:
            logger.debug("Found .spr.yaml, loading...")
            repo_config = yaml.safe_load(f)
            logger.debug(f"Config from .spr.yaml: {repo_config}")
            if repo_config:
                ...
                if 'user' in repo_config and isinstance(repo_config['user'], dict):
                    logger.debug(f"Adding user config: {repo_config['user']}")
                    config['user'].update(repo_config['user'])
    except FileNotFoundError:
        logger.debug("No .spr.yaml found, using defaults")
