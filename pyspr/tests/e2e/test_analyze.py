...
def create_commits_from_dag(
    dependencies: Dict[str, List[str]], commits_order: List[str]
) -> None:
...
def test_analyze_complex_dependencies(test_repo_ctx: RepoContext) -> None:
    ...
    Single Stack Algorithm (must be implemented exactly as specified):
    For each commit top-down,
        - Try cherry-picking to merge-base (removing it from the stack))
        - Or else leave it in place
    This gives you up to one single stack plus some independents, and never orphans. Submits all commits.

    Expected Single Stack output structure:
    ...
    assert "🔢 Single Stack: Remove Independents from Stack" in output
    ...
    independent_section = re.search(
        r"✅ Independent commits \(\d+\):(.*?)❌ Dependent commits", output, re.DOTALL
    )
    ...
    def verify_tree_structure(
        trees: Dict[str, List[Tuple[int, str]]], expected: Dict[str, List[str]]
    ) -> None:
    ...
    # Verify Single Stack scenario
    single_stack_match = re.search(
        r"🔢 Single Stack: Remove Independents from Stack.*?"
        r"Stack:\s*(.*?)\n.*?"
        r"Independents:\s*(.*?)\n",
        output,
        re.DOTALL
    )
    assert single_stack_match, "Could not find Single Stack section"
    
    stack_commits_str = single_stack_match.group(1).strip()
    independents_str = single_stack_match.group(2).strip()
    
    # Parse the commits from the strings
    stack_commits = [c for c in stack_commits_str.split() if c]
    independent_commits_single = [c for c in independents_str.split() if c]
    
    # Expected results per the spec
    expected_stack = ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L"]
    expected_independents_single = ["M"]
    
    assert stack_commits == expected_stack, (
        f"Single Stack: expected stack {expected_stack}, got {stack_commits}"
    )
    assert independent_commits_single == expected_independents_single, (
        f"Single Stack: expected independents {expected_independents_single}, got {independent_commits_single}"
    )
    
    log.info(f"✓ Single Stack: {len(stack_commits)} commits in stack, {len(independent_commits_single)} independents")
    ...
    log.info(f"✓ Single Stack: {len(stack_commits)} in stack, {len(independent_commits_single)} independents")
